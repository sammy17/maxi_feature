...
#ifndef _feature_featureHist_H_
#define _feature_featureHist_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "feature_featureHist_memcore.h"

SC_MODULE(feature_featureHist) {
...
protected:
    ...
    // buffer instances
    feature_featureHist_memcore* buffer[BufferCount];
...
public:
    ~feature_featureHist();
    SC_CTOR(feature_featureHist) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "feature_featureHist_memcore_U_";
            name += szTmp;

            buffer[i] = new feature_featureHist_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "feature_featureHist.trace");
            // ports
